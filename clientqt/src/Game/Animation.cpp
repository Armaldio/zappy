    ...
    void AnimatedSprite::setFrame(std::size_t newFrame, bool resetTime) {
        if (m_animation) {
            ...
            if (m_size.x || m_size.y)
            {
                ...
                m_vertices[1].position = sf::Vector2f(0.f, static_cast<float>(m_size.x));
                m_vertices[2].position = sf::Vector2f(static_cast<float>(m_size.x), static_cast<float>(m_size.y));
                m_vertices[3].position = sf::Vector2f(static_cast<float>(m_size.x), 0.f);
            }
            else
            {
                m_vertices[0].position = sf::Vector2f(0.f, 0.f);
